...
- [ ] 4.6 Watch a live deploy end-to-end (the spec's attended gate). — *Pending: push this branch and confirm `build → deploy(reusable) → ui-tests → publish-nuget` is green. Caveat: a feature branch only exercises the `application.template`/single-region/nonprod path; the `master.template`/multi-region/prod path is refactored but unexercised until a shared-infra merge (note against 6.2).*
- [ ] 4.7 External-consumer template extraction. — *OPEN: deploy.yml runs `sam build --template infrastructure/…`, which works for this repo's self-consume (the checkout contains `infrastructure/`) but NOT for an external consumer whose checkout lacks the templates. They need a step to extract templates from the resolved `YadaYada.AwsWebApp.DeploymentStack` NuGet (`contentFiles/any/any/infrastructure/`) before the deploy. Add that step (or document it in CONSUMING.md, Phase 4) before claiming the "callable from another repo" scenario is fully satisfied.*
