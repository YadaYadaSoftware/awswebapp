...
> **RECONCILIATION (task 1.3) — D2 is superseded.** `make-deployment-stack-reusable` shipped **derive-from-domain (§0.1): there is no `ProjectName` parameter or `project-name` input.** Resource naming derives from the **dashed domain**, so resources can only be namespaced by *domain*, not by a project axis. Consequences if the sample shares `appcloud.systems`: it shares TaskManager's bootstrap-owned KMS/ECR/SSM (named for the dashed domain), and a *feature-branch* sample imports **dev's backend Aurora** — so `SampleDbContext`'s Identity/`AspNetUsers` migration would land in TaskManager's dev database (collision). **Therefore clean coexistence requires the sample to have its own DOMAIN** (→ its own bootstrap stack → own KMS/ECR/SSM/Aurora/hosted zone), not merely a distinct `ProjectName`. This is a larger AWS-provisioning lift than D2 assumed and is the gap this change reports upstream. Pending the user's Q1/Q2 decision.
