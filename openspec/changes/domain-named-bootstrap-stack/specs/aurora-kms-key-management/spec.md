...
- **THEN** the deploy succeeds and creates: the shared infra (ECR, ApiGateway role+account, Config rule + Lambda + SSM doc, TemplatesBucketPolicy), both multi-region KMS keys (nonprod with broad policy, prod with a targeted Deny on `GitHubActionsUser`), both aliases (`alias/appcloud-systems-aurora-nonprod` and `alias/appcloud-systems-aurora-prod`), both SSM parameters (`/appcloud-systems/kms/nonprod/aurora-key-arn` and `/appcloud-systems/kms/prod/aurora-key-arn`), both IAM users (`GitHubActionsUser`, `GitHubActionsUserProd`) with their access keys and policies, and the `prod-kms-admin` role
