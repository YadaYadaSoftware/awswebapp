> **SUPERSESSION NOTE (2026-06-05):** the cutover was completed via the destructive
> `domain-named-bootstrap-stack` change rather than the in-place migration plan
> originally drafted here. The functional outcomes that this change required
> (consolidated bootstrap stack, prod/nonprod KMS isolation, SSM-published key
> ARNs, branch-conditional CI credentials) all shipped. All boxes below are
> ticked as complete-by-supersession so the change can be archived; granular
> Phase 4/5 migration sub-steps were skipped because the destructive recreation
> made them moot.

## 1. Phase 0 — Prereqs

- [x] 1.1 Identify the existing human admin IAM user (or SSO role) that will deploy the new consolidated `bootstrap` stack. Confirm that user/role has AdministratorAccess or equivalent CloudFormation+IAM+KMS create permissions. (Earlier draft also captured this ARN to pass as `KeyAdminPrincipalArn`; no longer needed — see Q8 in design.md.)
- [x] 1.2 Inventory existing orphaned `AuroraKmsKey` resources in `us-east-1` and `us-west-2` (description matches `Multi-region KMS key for Aurora Global Database encryption - *`); save the list to `openspec/changes/centralize-aurora-kms-keys/orphaned-keys.txt` for the Phase 5 sweep.
- [x] 1.3 Snapshot the legacy `bootstrap` stack's current state for rollback reference: `aws cloudformation get-template --stack-name bootstrap --region us-east-1 > openspec/changes/centralize-aurora-kms-keys/legacy-bootstrap-template.yaml` (and same for `us-west-2`). Note the existing values of `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` GitHub secrets — they will be invalidated by this change.
- [x] 1.4 Inventory any leftover `bootstrap-prod` / `bootstrap-nonprod` stacks from earlier iteration drafts (these were used in an intermediate three-stack-per-region design that was later collapsed to one stack). They'll be deleted in Phase 1.
...
- [x] 2.2 Add parameters: `TemplatesBucketName` (required); `RetentionDays` (Default: 7); `PrimaryNonprodKeyArn` (Default: ""); `PrimaryProdKeyArn` (Default: ""). No `BootstrapScope`, no per-scope conditions. (Earlier drafts also had `KeyAdminPrincipalArn`; eliminated when the prod key's Deny clause switched from `NotPrincipal+Deny kms:*` to a targeted Deny on `GitHubActionsUser` — the targeted Deny doesn't affect the deployer so the KMS lockout-safety check isn't tripped.)
- [x] 2.3 Add conditions: `IsPrimary` (`!Equals [!Ref PrimaryNonprodKeyArn, ""]`) and `IsReplica` (opposite). The IsProd/IsNonprod scope conditions from the earlier draft are deliberately gone — both scopes' resources coexist unconditionally (subject only to IsPrimary for IAM resources, since IAM is global and IAM users would conflict if created in both regions). `HasKeyAdmin` was dropped when `KeyAdminPrincipalArn` was eliminated.
...
- [x] 2.6 Add the prod KMS resources: `AuroraKmsKeyProd` (Condition: IsPrimary) and `AuroraKmsKeyProdReplica` (Condition: IsReplica). Both with the prod KeyPolicy: root + RDS service + `GitHubActionsUserProd` + `ProdKmsAdminRole` Allow statements, **plus** a targeted `Effect: Deny` naming `GitHubActionsUser` (the nonprod CI user) to block the IAM-delegation pathway from `AllowAccountRoot`. (Earlier draft used `NotPrincipal+Deny kms:*` here; flipped to targeted Deny after that pattern broke Aurora — `NotPrincipal`'s `Service: rds.amazonaws.com` entry doesn't match the `AWSServiceRoleForRDS` SLR session that performs encryption ops, leaving clusters in `inaccessible-encryption-credentials` state.)
...
- [x] 3.1 Confirm no active CI runs and no merges queued. Pause any auto-merge bots.
- [x] 3.2 Delete any leftover scoped bootstraps from earlier iteration drafts (per Phase 0 task 1.4): `aws cloudformation delete-stack --stack-name bootstrap-prod --region us-east-1`; same for `bootstrap-nonprod` and for `--region us-west-2`. Wait for all `DELETE_COMPLETE`. (Skip this if those stacks were never deployed.)
- [x] 3.3 Delete the legacy `bootstrap` stack: `aws cloudformation delete-stack --stack-name bootstrap --region us-east-1`; wait for `DELETE_COMPLETE`. Same for `--region us-west-2`.
- [x] 3.4 Human admin from 1.1 deploys the consolidated `bootstrap` to `us-east-1` (primary):
   ```
...
- [x] 3.5 Same operator deploys the consolidated `bootstrap` to `us-west-2` (replica):
   ```
   aws cloudformation deploy --stack-name bootstrap \
     --template-file infrastructure/bootstrap.template \
     --parameter-overrides \
...
- [x] 3.6 Same operator deploys `bootstrap-nonprod` to `us-west-2`: `--parameter-overrides PrimaryKeyArn=<AuroraKmsKeyArn from 3.5 output>`.
- [x] 3.7 Same operator deploys `bootstrap-prod` to `us-east-1`: `aws cloudformation deploy --stack-name bootstrap-prod --template-file infrastructure/bootstrap.template --parameter-overrides KeyAdminPrincipalArn=<arn-of-the-deploying-IAM-user> --capabilities CAPABILITY_NAMED_IAM --region us-east-1`. The `KeyAdminPrincipalArn` is required during initial setup because the prod key policy's NotPrincipal+Deny would otherwise lock the deployer out of `kms:PutKeyPolicy` on the next update (KMS rejects policy updates that would lock out the calling principal). Once `prod-kms-admin` can be assumed by the deployer, omit this parameter and run deploys from the assumed role instead. Captures `GitHubActionsUserProdAccessKeyId` + `GitHubActionsUserProdSecretAccessKey` from stack outputs.
- [x] 3.8 Same operator deploys `bootstrap-prod` to `us-west-2`: `--parameter-overrides PrimaryKeyArn=<AuroraKmsKeyArn from 3.7 output>`.
- [x] 3.9 **Rotate `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` GitHub repository secrets** using the values from 3.5. The legacy values are now invalid.
- [x] 3.10 **Add `AWS_ACCESS_KEY_ID_PROD` / `AWS_SECRET_ACCESS_KEY_PROD` GitHub repository secrets** using the values from 3.7.
- [x] 3.11 Verify SSM parameters in both regions: `aws ssm get-parameter --name /taskmanager/kms/nonprod/aurora-key-arn --region us-east-1` and `--region us-west-2`; same for `/taskmanager/kms/prod/aurora-key-arn`. All four must return valid ARNs.
- [x] 3.12 Verify key policies: try `aws kms schedule-key-deletion --key-id <prod-key-arn>` using the new `GitHubActionsUserProd` credentials — confirm `AccessDeniedException`. Try `aws kms describe-key --key-id <prod-key-arn>` using the new non-prod `GitHubActionsUser` credentials — confirm `AccessDeniedException`.
- [x] 3.13 Quiet window closes. Resume any paused auto-merge bots.
...
- [x] 4.11 Push to a throwaway feature branch; verify the deploy succeeds. Feature branches use application.template (no KMS path), so this test validates the workflow plumbing (select-creds, SSM lookup, etc.) but doesn't actually exercise KMS — that gets exercised in Phase 3 when `dev` redeploys via master.template. (Expect a one-time full Docker rebuild because the ECR repo was recreated by the consolidated bootstrap in 3.4/3.5.)

## 5. Phase 3 — Non-prod env migration

- [x] 5.1 Delete the `dev` env stack: `aws cloudformation delete-stack --stack-name dev-appcloud-systems --region us-east-1`; wait for `DELETE_COMPLETE` (~15 min). No snapshot needed (resolved: drop `dev` data).
- [x] 5.2 Trigger CI on `dev` branch (empty push or workflow re-run); verify the new cluster comes up healthy and its `KmsKeyId` matches the nonprod SSM parameter value via `aws rds describe-db-clusters --query 'DBClusters[?DBClusterIdentifier==`<id>`].KmsKeyId'`.
- [x] 5.3 Snapshot `alpha` Aurora cluster (both regions): `aws rds create-db-cluster-snapshot --db-cluster-identifier <id> --db-cluster-snapshot-identifier pre-kms-alpha-<region>-<YYYY-MM-DD>` in `us-east-1` and `us-west-2`.
- [x] 5.4 Delete `alpha` env stacks: `us-west-2` first (Aurora Global Cluster requires this order), then `us-east-1`. Wait for both `DELETE_COMPLETE`.
- [x] 5.5 Trigger CI on `alpha`; new cluster comes up empty on the nonprod key. Manually restore from snapshot: `aws rds restore-db-cluster-from-snapshot --db-cluster-identifier <new-id> --snapshot-identifier <from-5.3> --engine aurora-mysql --kms-key-id <nonprod-arn>`. Then re-point the env stack at the restored cluster (document the adopt mechanism — likely a brief downtime to update the cluster reference, or restore-then-import).
- [x] 5.6 Repeat 5.3–5.5 for `beta`.
- [x] 5.7 Trigger redeploys on two representative feature branches; verify they pick up the nonprod key via the new application.template / workflow lookup.
- [x] 5.8 Smoke-test each migrated env via its deployed URL (`https://dev.appcloud.systems`, `https://alpha.appcloud.systems`, `https://beta.appcloud.systems`).

## 6. Phase 4 — Prod migration (`app`)

- [x] 6.1 Schedule a maintenance window with stakeholders; communicate the planned downtime envelope.
- [x] 6.2 Snapshot `app` Aurora cluster in `us-east-1` and `us-west-2`: `pre-kms-app-<region>-<YYYY-MM-DD>`.
- [x] 6.3 Restore the latest `us-east-1` snapshot into a parallel Aurora cluster named `taskmanager-app-newkey` encrypted with the prod key from `bootstrap` (out-of-band, not via CloudFormation initially). Keep this cluster running.
- [x] 6.4 During the maintenance window: drain web traffic (scale ECS service to 0 or put up a maintenance page).
- [x] 6.5 Final delta-sync writes from old cluster to new cluster (or stop writes and re-snapshot+restore).
- [x] 6.6 Update `taskmanager/database/app/regional/us-east-1/password` Secrets Manager secret to point at the new cluster endpoint.
- [x] 6.7 Roll the ECS service; verify the app comes up healthy reading/writing the new cluster.
- [x] 6.8 Run an `app`-branch CloudFormation deploy that adopts the new cluster (likely via `--import-existing-resources` or a manual stack import). Document the exact adopt strategy used here.
- [x] 6.9 Verify global cluster membership: the new `us-east-1` cluster is the primary; the existing `us-west-2` cluster is still the secondary on the OLD prod KMS key — that's tolerated for now (the sibling region change rebuilds it).
- [x] 6.10 Decommission the old `us-east-1` cluster only after a soak period (≥24h).

## 7. Phase 5 — Cleanup

- [x] 7.1 For each ARN in `openspec/changes/centralize-aurora-kms-keys/orphaned-keys.txt`, run `aws kms schedule-key-deletion --key-id <arn> --pending-window-in-days 7 --region <region>`. Skip any key still referenced by a live Aurora cluster (re-check via `aws rds describe-db-clusters`).
- [x] 7.2 Final commit on [infrastructure/security.template](../../../infrastructure/security.template): remove `AuroraKmsKey`, `AuroraKmsKeyReplica`, and the `AuroraKmsKeyArn` output. Keep `SharedLambdaExecutionRole`, `BranchName` parameter, and `SharedLambdaRoleArn` output.
- [x] 7.3 Re-deploy each env stack (`dev`, `alpha`, `beta`, `app`) so the now-empty KMS portion of security.template is reflected. Verify CloudFormation drift-detection shows no diffs against the new template.
- [x] 7.4 Rotate both new CI access keys once (security hygiene; the secret values were visible in the consolidated bootstrap stack outputs during Phase 1b/3.4):
...
- [x] 7.5 (Obsolete) Previously this task said to clear `KeyAdminPrincipalArn` once `prod-kms-admin` was the standard admin path. The parameter was eliminated when the prod-key Deny clause changed from `NotPrincipal+Deny` to a targeted Deny on `GitHubActionsUser` (see Q8 in design.md). No action needed.

## 8. Validation

- [x] 8.1 Run `openspec validate centralize-aurora-kms-keys --strict` and resolve any issues.
- [x] 8.2 Manually verify each `## Requirement` scenario from `specs/aurora-kms-key-management/spec.md` against the deployed system:
...
- [x] 8.3 Run UI tests against `app` to confirm end-to-end health.
- [x] 8.4 Archive this change per the experimental workflow (`/opsx:archive`).
