...
**`AllowAccountRoot` is paired with a targeted Deny on `GitHubActionsUser`.** Without the Deny, the standard `AllowAccountRoot kms:* Resource:*` statement delegates evaluation to IAM, and the nonprod CI user's broad `DeploymentPolicy` (`kms:*` on `Resource:"*"`) reaches the prod key — including decrypting prod data. The targeted Deny:

```yaml
- Sid: DenyNonprodCI
  Effect: Deny
  Principal:
    AWS: !GetAtt GitHubActionsUser.Arn      # !Sub'd ARN in the replica region
  Action: kms:*
  Resource: "*"
```

closes that gap by naming the specific untrusted principal — nothing else is denied.

**Why a targeted Deny instead of `NotPrincipal+Deny`?** Earlier drafts used `Effect: Deny` with `NotPrincipal` listing an exemption set (root, `GitHubActionsUserProd`, `prod-kms-admin`, `rds.amazonaws.com`). That seemed cleaner (deny-by-default), but it broke Aurora cluster creation: a `NotPrincipal` entry of `Service: rds.amazonaws.com` only matches *direct* service-principal calls, **not** the assumed-role session of `AWSServiceRoleForRDS` — and the SLR is what actually performs the cluster's encryption operations once RDS has created the grant. The cluster ended up in `inaccessible-encryption-credentials` state until the Deny was inverted. The targeted-Deny shape doesn't deny anyone it shouldn't, so the SLR (and any future AWS service-linked role) is unaffected. The trade-off is that *new* untrusted IAM principals would need to be explicitly added to the Deny — acceptable because new IAM users in this account are created deliberately, not spontaneously. (Discovered during the first `app` deploy — see the design retrospective comment in [bootstrap.template](../../../infrastructure/bootstrap.template) for the SLR-vs-Service-principal mechanics.)

**No `KeyAdminPrincipalArn` parameter is needed.** Earlier drafts required one because `NotPrincipal+Deny kms:*` blocked the deployer too, tripping KMS's lockout-safety check on every policy update. The targeted Deny only names `GitHubActionsUser`, so the deployer's `kms:PutKeyPolicy` is unaffected and the lockout check passes naturally. Deploy command for `bootstrap` is correspondingly simpler — no extra parameter override needed.
...
- **Q8 — `NotPrincipal+Deny` or targeted Deny for the prod key boundary?** **Targeted Deny.** First-try draft used `NotPrincipal+Deny kms:*` with an AWS+Service exemption list (root, `GitHubActionsUserProd`, `prod-kms-admin`, `rds.amazonaws.com`). The first `app` deploy failed at `AuroraCluster` with `inaccessible-encryption-credentials` because RDS performs encryption ops via the `AWSServiceRoleForRDS` SLR session, and a `Service: rds.amazonaws.com` entry in `NotPrincipal` doesn't match SLR sessions. Flipped to a targeted Deny on the specific untrusted principal (`GitHubActionsUser`). This: (a) doesn't affect AWS-internal service-linked roles, (b) doesn't trip KMS's lockout-safety check on the deployer, and (c) consequently lets us delete the `KeyAdminPrincipalArn` parameter and the `HasKeyAdmin` condition entirely. Trade-off accepted: any future "untrusted" IAM user would need to be explicitly added to the Deny.

- **Q9 — Move `SharedLambdaExecutionRole` to bootstrap and delete `security.template` entirely?** **Deferred.** After the Section 4 cleanup, `security.template` contains only `SharedLambdaExecutionRole` and its `SharedLambdaRoleArn-${BranchName}` export. Conceptually the role belongs in bootstrap (IAM is global; per-env duplication has identical policies and zero cost benefit). But the export is `Fn::ImportValue`'d by [api.template](../../../infrastructure/api.template) and [web.template](../../../infrastructure/web.template) using the `-${EnvironmentToImport}` suffix pattern; moving the role to bootstrap requires threading a new `SharedLambdaRoleArn` parameter through the whole chain (workflow SSM lookup → master → application → api/web) and rewriting those `Fn::ImportValue`s to `!Ref`. That's a 7-file refactor and right now we're trying to push dev to validate this KMS change end-to-end — adding more in-flight refactor risk would conflate failure causes. Tracked as a follow-up in [tasks.md](tasks.md#9-follow-ups-after-this-change-archives). Spec/proposal unchanged because the deferred work is a future change, not a property of this one.
