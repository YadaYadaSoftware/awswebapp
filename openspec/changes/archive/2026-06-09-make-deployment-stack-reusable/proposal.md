...
- Replace hardcoded project-specific values (`taskmanager`, `appcloud.systems`, container image paths, etc.) with CloudFormation template parameters: `ProjectName`, `DomainName`, `WebContainerImage`, and similar. The IAM policies that scope resources to `taskmanager-*` become `${ProjectName}-*` via `!Sub`. The branch model (shared-infrastructure branches: `app`/`beta`/`alpha`/`dev`; feature branches everywhere else) stays hardcoded *for this change* (configurable is a future extension).
...
- Every template in [infrastructure/](../../../infrastructure/) — hardcoded `taskmanager` / `appcloud.systems` / etc. replaced with `!Sub` references to new template parameters.
