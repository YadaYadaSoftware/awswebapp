...
- **Database**: MySQL via `Pomelo.EntityFrameworkCore.MySql` (not PostgreSQL). Production runs Aurora MySQL Serverless v2 (Global Cluster on `app`/`test`). README mentions PostgreSQL/RDS — ignore.
...
- **Forking into a new repo**: the root README's "Setting up a new repo" section is the fork onboarding guide (openspec change `standalone-fork-readiness`). `sample-deploy.yml` has a `validate-config` preflight that fails fast on missing required Variables/Secrets, reads the hosted zone from `vars.HOSTED_ZONE_ID` (TaskManager's main deploy keeps the literal in `zbuild.yml`; set the Variable only if you enable `sample-deploy` here), and sources the framework-feed token from `FRAMEWORK_FEED_TOKEN` (a cross-org `read:packages` PAT) **falling back to `GITHUB_TOKEN`** when unset — so a same-org caller needs nothing new.
...
| `Branch` | branch leaf |
| `Specification` | branch leaf (= OpenSpec change name for feature branches); `shared-infrastructure` on `app`/`test`/`dev` |
...
- Two "shared infrastructure" branches deploy multi-region (`AWS_REGION_PRIMARY` + `AWS_REGION_SECONDARY` repo vars; currently `us-east-1` + `us-east-2`): `app`, `test`. They use `infrastructure/master.template` (full backend incl. Aurora Global Cluster).
...
- The `dev`/`test`/`app` branches **fail the build if `changes/` is non-empty** — use `scripts/merge-to-dev.ps1` (or the `.sh` variant) to flush pending change files into `changelog.md`.
...
- The shared-infrastructure branches (`app`/`test`/`dev`) which serve their own purposes.
...
- `app`, `test`, and `dev` are not brothers — they are the **homestead** (shared long-lived worktrees); `/brothers` lists them separately. `.bare` is the hub, not a folder you work in.
...
- **UI tests share suite-level fixtures + opt-in retries (`stabilize-ui-tests`).** Every class in `src/Tjb.UiTests/` inherits **`BaseUiTest`** (not the old `BaseTest`, which is gone) and carries `[Collection("UiTests")]`. The collection wires two `IAsyncLifetime` fixtures that run ONCE per suite: **`OAuthTokenFixture`** (mints a fresh Google access token from `GOOGLE_TEST_REFRESH_TOKEN` + `GOOGLE_OAUTH_CLIENT_ID`/`GOOGLE_OAUTH_CLIENT_SECRET`, re-minting at >50min via `RefreshIfStaleAsync`; tests read `AccessToken` off the base, never the env var directly) and **`AppReadinessFixture`** (polls `${TEST_BASE_URL}/health` every 2s for up to 5min so cold-start/ALB warmup doesn't cascade). `BaseUiTest` gives each test a `Page` pre-set to `DefaultTimeout=30s` / `DefaultNavigationTimeout=60s`. Assertions go through `Expect(...).ToBe*Async(new(){Timeout=10_000})` (helpers on the page objects), not instantaneous `IsVisibleAsync`/synchronous reads or fixed sleeps. **New tests must inherit `BaseUiTest` + add `[Collection("UiTests")]` + forward both fixtures via the constructor.** Genuinely-flaky tests opt into `[RetryFact(MaxRetries = 2)]` (from `xRetry`) — never blanket-apply, that masks regressions.
- **Authenticated-UI tests sign in via a gated test-auth endpoint (`ui-test-authenticated-session`).** A raw Google token in the `.AspNetCore.Identity.Application` cookie does NOT authenticate (the app correctly ignores it — verified byte-for-byte). So tests that need a *logged-in* page call `TrySignInViaTestAuthAsync()`, which POSTs the Google **`id_token`** (`OAuthTokenFixture.IdToken`) to **`POST /test-auth/signin`** (and skips the test on a 404, i.e. where the gate is off). That endpoint ([src/Tjb.Web/TestAuth/TestAuthEndpoint.cs](src/Tjb.Web/TestAuth/TestAuthEndpoint.cs)) validates the id_token (signature/`iss`/`aud`=our Google client/`exp` via `Google.Apis.Auth`), provisions/resolves the Identity user, and issues a real cookie via `SignInManager`. It is **gated by `TestAuth:Enabled`** and only mapped when true — [infrastructure/web.template](infrastructure/web.template) sets `TestAuth__Enabled` **false on the production branch `app`, true everywhere else (`dev`, `test`, feature stacks)** via the `TestAuthDisabled` condition, so it is 404 in production. Never enable it on `app`.
- **Deleting a remote branch tears down its CloudFormation stack.** [.github/workflows/cleanup-on-branch-delete.yml](.github/workflows/cleanup-on-branch-delete.yml) fires on the GitHub `delete` event, computes the same `{branch-leaf}-{processed-domain}` stack name the deploy job uses, and calls `aws cloudformation delete-stack` in `us-east-1`. `app`, `test`, and `dev` are hard-coded as protected — the workflow no-ops for any branch whose leaf segment matches one of those (including e.g. `feature/dev`). It also clears `s3://${AccountId}-${DashedDomain}-us-east-1/{branch-leaf}/` (the bootstrap-owned templates bucket — see Infrastructure naming convention above) after `DELETE_COMPLETE`. **Pre-cleanup (robust-branch-stack-cleanup):** before `delete-stack`, the workflow now enumerates the full stack tree (recursing nested stacks via `list-stack-resources`) and preemptively empties every stack-owned S3 bucket, deletes every stack-owned ECR repo's images, and force-tears-down every stack-owned Aurora cluster (delete members → clear deletion-protection → `delete-db-cluster --skip-final-snapshot` → wait). It acts ONLY on CFN-tracked members of the stack being deleted — the bootstrap's `TemplatesBucket`/`WebECRRepository`/KMS keys are never touched — and halts before `delete-stack` if any pre-cleanup step fails, so operators no longer hand-clean these before retrying a stuck delete. **GitHub Actions constraint — default-branch-only workflows (gotcha):** GitHub dispatches repo-level events like `delete` **and `schedule`** using the workflow file *as it exists on the default branch* (`app`) — never the copy on `dev` or a feature branch. So edits to `cleanup-on-branch-delete.yml` (or any `delete`/`schedule`-triggered workflow) are integrated but **dormant** until a deliberate `dev`→`app` promotion; e.g. the `robust-branch-stack-cleanup` pre-cleanup logic above does not actually run until it reaches `app`. The `default-branch-workflow-promotion` change adds a CI drift guard that flags, on each non-`app` run, any such workflow that differs from `app` (a "dormant until promoted" notice on the run summary).
