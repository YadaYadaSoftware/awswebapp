...
- **Authenticated-UI tests sign in via a gated test-auth endpoint (`ui-test-authenticated-session`).** A raw Google token in the `.AspNetCore.Identity.Application` cookie does NOT authenticate (the app correctly ignores it — verified byte-for-byte). So tests that need a *logged-in* page call `SignInViaTestAuthAsync()`, which POSTs the Google **`id_token`** (`OAuthTokenFixture.IdToken`) to **`POST /test-auth/signin`**. That endpoint ([src/Tjb.Web/TestAuth/TestAuthEndpoint.cs](src/Tjb.Web/TestAuth/TestAuthEndpoint.cs)) validates the id_token (signature/`iss`/`aud`=our Google client/`exp` via `Google.Apis.Auth`), provisions/resolves the Identity user, and issues a real cookie via `SignInManager`. It is **gated by `TestAuth:Enabled`** and only mapped when true — [infrastructure/web.template](infrastructure/web.template) sets `TestAuth__Enabled` **true on dev + feature stacks, false on `app`/`beta`/`alpha`** (the `TestAuthDisabled` condition), so it is 404 in production. Never enable it there.
